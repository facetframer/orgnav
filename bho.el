...
(defun bho--make-source (default-action)
  (list
   (cons 'name "HELM at the Emacs")
   (cons 'candidates 'bho--get-candidates)
...
(defun bho--get-candidates ()
  "Helm candidate function"
  (save-excursion
     ...
     (let ((current-level
            (if (null bho-var-point) 0
              (save-excursion
                ...
                (org-outline-level)))))
       (bho--filter-by-depth
        (bho-get-children bho-var-point)
...
"
  ...
  ;;   not be run in the current buffer
  ...
  (interactive)
  (helm :sources (list (bho--make-source default-action)) :keymap bho-mapping :buffer buffer-name))
